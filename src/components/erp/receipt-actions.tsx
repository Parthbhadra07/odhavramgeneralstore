...
export function ReceiptActions({
  ...
  useEffect(() => {
    ...
    window.addEventListener("ogs-printer-settings-changed", refresh);
    window.addEventListener("ogs-barcode-printer-changed", refresh);
    return () => {
      window.removeEventListener("ogs-printer-settings-changed", refresh);
      window.removeEventListener("ogs-barcode-printer-changed", refresh);
    };
  }, []);

  const printOptions = getReceiptPrintOptions(width);
        ...
        >
          ...
          >
            ...
            <div className="receipt-preview-scope">
              <style key={styleKey}>{getReceiptPreviewStyles(width, printOptions)}</style>
              <ReceiptPrint
                data={data}
                settings={settings}
                width={width}
